...
import { ArrowLeft, Upload, FileSpreadsheet, FileText, Loader2, CheckCircle2, X, AlertCircle } from "lucide-react";
...
const ACCEPTED_TYPES = [".xlsx", ".xls", ".csv", ".txt", ".docx", ".pdf", ".json"];
...
function normalizeHeaders(headers: string[]): Record<string, number> {
  ...
  headers.forEach((h, i) => {
    ...
    for (const [field, names] of Object.entries(aliases)) {
      if (names.includes(clean) && !(field in map)) map[field] = i;
    }
  });
  return map;
}
...
function rowsToQuestions(rows: string[][]): { questions: ParsedQuestion[]; errors: string[] } {
  ...
  rows.slice(1).forEach((row, idx) => {
    ...
    if (!q) { errors.push(`Row ${idx + 2}: Missing question text`); return; }
    const answer = get("correct_answer").toUpperCase();
    if (!["A", "B", "C", "D"].includes(answer)) { errors.push(`Row ${idx + 2}: Invalid answer "${answer}"`); return; }
    questions.push({ topic: get("topic") || "General", question: q, option_a: get("option_a"), option_b: get("option_b"), option_c: get("option_c"), option_d: get("option_d"), correct_answer: answer });
  });
...
async function parseFile(file: File): Promise<{ rows: string[][]; errors: string[] }> {
  ...
  if (ext === "json") {
    ...
    try {
      ...
      return { rows, errors };
    } catch { return { rows: [], errors: ["Invalid JSON format"] }; }
  }
  ...
  if (ext === "docx") {
    try {
      ...
      return { rows: [], errors: ["No table found in Word document."] };
    } catch { return { rows: [], errors: ["Failed to parse Word document"] }; }
  }
...
const ImportQuestions = () => {
  ...
  const handleFile = useCallback(async (file: File) => {
    setParsing(true); setFileName(file.name); setParsedQuestions([]); setParseErrors([]); setImported(false);
    try {
      const { rows, errors: fileErrors } = await parseFile(file);
      if (fileErrors.length > 0 && rows.length === 0) { setParseErrors(fileErrors); setParsing(false); return; }
      const { questions, errors: rowErrors } = rowsToQuestions(rows);
      setParsedQuestions(questions);
      setParseErrors([...fileErrors, ...rowErrors]);
    } catch { setParseErrors(["Failed to read file. Please check the format."]); }
    setParsing(false);
  }, []);
  ...
  const downloadTemplate = (format: "xlsx" | "csv") => {
    ...
    XLSX.writeFile(wb, format === "xlsx" ? "question_template.xlsx" : "question_template.csv");
  };

  const reset = () => { setParsedQuestions([]); setParseErrors([]); setFileName(""); setImported(false); if (fileRef.current) fileRef.current.value = ""; };
  ...
  return (
    <div className="min-h-screen px-4 py-6 max-w-2xl mx-auto page-bg">
      ...
      <div className="mb-6">
        <h1 className="text-xl font-bold mb-1 font-display">Import Questions</h1>
      ...
      <Card className="mb-4 rounded-2xl border-0 shadow-sm">
        <CardContent className="py-4 px-5">
          <p className="text-xs font-semibold mb-3 font-display">Download Template</p>
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" size="sm" className="text-xs h-9 gap-1.5 rounded-full" onClick={() => downloadTemplate("xlsx")}>
              <FileSpreadsheet className="w-3.5 h-3.5" /> Excel Template
            </Button>
            <Button variant="outline" size="sm" className="text-xs h-9 gap-1.5 rounded-full" onClick={() => downloadTemplate("csv")}>
      ...
      {!imported ? (
        <>
          {/* Upload area */}
          <Card
            className="mb-4 border-dashed border-2 border-border cursor-pointer hover:border-primary/40 transition-colors rounded-2xl"
          ...
          >
            <CardContent className="py-12 flex flex-col items-center text-center">
              ...
              <Input ref={fileRef} type="file" className="hidden" accept={ACCEPTED_TYPES.join(",")} onChange={(e) => { const file = e.target.files?.[0]; if (file) handleFile(file); }} />
            </CardContent>
          </Card>

          {/* Errors */}
          {parseErrors.length > 0 && (
            <Card className="mb-4 border-destructive/20 bg-destructive/5 rounded-2xl">
              <CardContent className="py-4 px-5">
                <div className="flex items-start gap-2.5">
                  ...
                  <div>
                    ...
                    <ul className="space-y-0.5">
                      {parseErrors.slice(0, 10).map((err, i) => (<li key={i} className="text-[10px] text-destructive/80">{err}</li>))}
                      {parseErrors.length > 10 && <li className="text-[10px] text-destructive/60">…and {parseErrors.length - 10} more</li>}
                    </ul>
                  </div>
                </div>
              </CardContent>
            ...
            <Card className="mb-4 rounded-2xl border-0 shadow-sm">
              <CardHeader className="py-4 px-5 pb-2">
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle className="text-sm font-display">Preview</CardTitle>
                    <CardDescription className="text-[10px]">{parsedQuestions.length} questions ready to import</CardDescription>
                  </div>
                  <div className="flex gap-2">
                    <Button variant="ghost" size="sm" className="text-xs h-8 rounded-full" onClick={reset}><X className="w-3 h-3 mr-1" /> Clear</Button>
                    <Button size="sm" className="text-xs h-8 font-semibold btn-primary" onClick={handleImport} disabled={uploading}>
                    ...
                    <TableBody>
                      {parsedQuestions.slice(0, 20).map((q, i) => (
                        <TableRow key={i}>
                          ...
                          <TableCell className="text-[10px]"><span className="chip-primary text-[9px] px-2 py-0.5">{q.topic}</span></TableCell>
                          <TableCell className="text-[10px] max-w-[300px] truncate">{q.question}</TableCell>
                          <TableCell className="text-[10px] text-center font-bold text-primary">{q.correct_answer}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
                </div>
                {parsedQuestions.length > 20 && <p className="text-center text-[10px] text-muted-foreground py-2">Showing 20 of {parsedQuestions.length} questions</p>}
              </CardContent>
            </Card>
          )}
        </>
        ...
        <Card className="rounded-2xl border-0 shadow-sm">
          <CardContent className="py-12 text-center">
            ...
            <h2 className="text-lg font-bold mb-1 font-display">{importCount} Questions Imported!</h2>
            <p className="text-xs text-muted-foreground mb-6">Questions are now available in your question bank</p>
            <div className="flex flex-col sm:flex-row gap-2 justify-center">
              <Link to="/questions"><Button size="sm" className="gap-1.5 text-xs btn-primary">View Question Bank</Button></Link>
              <Button variant="outline" size="sm" className="gap-1.5 text-xs rounded-full" onClick={reset}>Import More</Button>
            </div>
          </CardContent>
        </Card>
      )}
