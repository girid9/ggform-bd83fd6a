  ...
  useEffect(() => {
    const load = async () => {
      ...
      data.forEach((a: any) => {
        ...
        if (!map[key]) {
          map[key] = { student_name: a.student_name, bestScore: 0, totalQuestions: a.total_questions, attempts: 0 };
        }
        map[key].attempts++;
        const pct = Math.round((a.score / a.total_questions) * 100);
        if (pct > map[key].bestScore) {
          map[key].bestScore = pct;
          map[key].student_name = a.student_name;
          map[key].totalQuestions = a.total_questions;
        }
      });
          ...
          {entries.slice(0, 10).map((e, i) => (
            <div key={e.student_name} className="flex items-center gap-3 text-sm">
              <span className="w-6 text-center">
                {i < 3 ? (
                  <Medal className={`w-4 h-4 inline ${medalColors[i]}`} />
                ) : (
              ...
              <span className={`text-xs font-bold ${e.bestScore >= 60 ? "text-success" : "text-destructive"}`}>
                {e.bestScore}%
              </span>
            </div>
          ))}
